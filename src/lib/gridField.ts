...
export class GridField {
  ...
  update(view: Float32Array, particleCount: number, halfW: number, halfH: number) {
    ...
    this.halfW = halfW;
    this.halfH = halfH;
    this.cols = Math.max(2, Math.ceil((2 * halfW) / spacing) + 1);
    this.rows = Math.max(2, Math.ceil((2 * halfH) / spacing) + 1);
    const n = this.cols * this.rows;
    if (this.points.length < n * FLOATS_PER_GRID_VERTEX) {
      this.points = new Float32Array(n * FLOATS_PER_GRID_VERTEX);
    }

    const eps2 = GRID_FIELD_EPS2;
    const pts = this.points;
    for (let r = 0; r < this.rows; r++) {
      const vy = -halfH + r * spacing;
      for (let c = 0; c < this.cols; c++) {
        const vx = -halfW + c * spacing;
        let ax = 0;
        let ay = 0;
        let phi = 0;
